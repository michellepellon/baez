...
## Key Symbols
- **`to_markdown(raw, meta, doc_id, notes, summary_text, related, status)`** → `Result<MarkdownOutput>`
  - Signature now includes `related: Vec<String>` (entity wiki-links) and `status: Option<&str>` (transcript quality: `"substantive"` | `"stub"`)
  - Builds `Frontmatter` with all fields including `related`, `status`
...
- **`convert_node` / `render_inline`** (private) — recursive ProseMirror walkers
...
## Test Modules
- `tests`, `snapshot_tests`, `prosemirror_convert_tests`
